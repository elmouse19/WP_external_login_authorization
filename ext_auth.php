...
add_shortcode('ext_auth','ext_auth_page'); // comment this out in production

wp_enqueue_script( 'ext_auth', plugin_dir_url( __FILE__ ) . 'ext_auth.js', array( 'jquery' ) );

add_filter( 'authenticate', 'ext_user_auth', 10, 3 ); // override wordpress's authentication
...
function ext_user_auth( $user, $username, $password ){
  // called by wp when someone posts to the login page
  error_log('ext_user_auth()');
